...
harness, not the generated code: the Z80 debug host's DCC-derived `FullCpmHost` model
...
generated bytes both passed and failed). `test_z80_debug_host.py` now
...
and completes a BIOS warm reload under `z80-debug-host` using the SBC adapter. More invasive
