...
{
    return SleepConditionVariableSRW(condition, mutex, INFINITE, 0) ? 0 : -1;
}
...
static bool chat_is_closing(void)
{
    bool closing;
...
static void chat_worker_finished(void)
{
    pthread_mutex_lock(&worker_lock);
    if (active_workers != 0)
        --active_workers;
...
static int curl_progress_cb(void* context, curl_off_t download_total,
...
static void trigger_request(void)
{
    ...
    if (chat_closing)
    {
        ...
        free(worker_arg);
        free(body);
        ring_put_str(generation, "Chat adapter is closing\n");
        ring_set_eof(generation);
        return;
    }
    ++active_workers;
