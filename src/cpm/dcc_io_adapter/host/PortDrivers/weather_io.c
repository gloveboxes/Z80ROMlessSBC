...
#ifdef _WIN32
    pthread_mutex_unlock(&s_mutex);
    WaitForSingleObject(s_stop_event, (DWORD)(seconds * 1000));
    pthread_mutex_lock(&s_mutex);
#else
    struct timespec ts;
...
#ifdef _WIN32
    if (WaitForSingleObject(s_stop_event, 2000) == WAIT_OBJECT_0)
        return NULL;
#else
...
void weather_io_init(void)
{
    ...
    {
        weather_set_error("CreateEvent failed");
        return;
    }
#endif
...
void weather_io_close(void)
{
    if (!s_initialized)
        return;
#ifdef HAVE_LIBCURL
    pthread_mutex_lock(&s_mutex);
    bool join_thread = s_thread_started;
    s_thread_started = false;
#ifdef _WIN32
    if (s_stop_event != NULL)
